...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  /* there are remain region memory after increase vma and allocate for the new region*/
...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  while(vma != NULL){
    if(vma->vm_id != vmaid){
      if(vma->vm_end > vmastart && vma->vm_end <= vmaend){
        return -1;
      }
      if ( vma->vm_start < vmaend && vma->vm_start >= vmastart){
        return -1;
      }
    }
    vma = vma->vm_next;
